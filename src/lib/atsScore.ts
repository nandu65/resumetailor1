// Browser mirror of supabase/functions/_shared/atsScore.ts
// Keep these in sync — same algorithm, same output.
...
export function computeAtsScore(resumeText: string, jdText: string): AtsScoreResult {
  ...
  const recruiter =
    Math.round(verbRatio * 35) +
    Math.round(impactPct * 30) +
    Math.round(structPct * 15) +
    Math.round(lengthOk * 10) +
    Math.round(Math.min(1, matched.length / 12) * 10);
